AmCharts.AmMap = AmCharts.Class({
    ...
    initChart: function() {
        ...
        } else(AmCharts.AmMap.base.initChart.call(this), AmCharts.hasSVG) ? (this.dataChanged && (this.parseData(), this.dispatchDataUpdated = !0, this.dataChanged = !1, a = this.legend) && (a.position = "absolute", a.invalidateSize()), this.mouseWheelZoomEnabled && this.addMouseWheel(), this.createDescriptionsDiv(), this.svgAreas = [], this.svgAreasById = {}, this.drawChart()) : (document.createTextNode(this.svgNotSupported), this.chartDiv.style.textAlign = "", this.chartDiv.setAttribute("class", "ammapAlert"), this.chartDiv.innerHTML = this.svgNotSupported, this.fire("failed", {
            type: "failed",
            chart: this
    ...
    drawBg: function() {
        ...
        var b = AmCharts.rect(a.container, a.realWidth, a.realHeight, "#000", .001);
        b.click(function() {
    ...
    handleMouseMove: function(a) {
        ...
        if (!isNaN(h)) {
            b.stopDrag();
            a.preventDefault && a.preventDefault();
            var h = Math.sqrt(Math.pow(h - e, 2) + Math.pow(l - f, 2)),
                m = b.previousDistance,
                l = Math.max(b.realWidth, b.realHeight);
            5 > Math.abs(m -
                h) && (b.isDragging = !0);
            if (!isNaN(m)) {
                var n = 5 * Math.abs(m - h) / l,
                    l = k.scale,
                    l = AmCharts.fitToBounds(m < h ? l + l * n : l - l * n, g.minZoomLevel, g.maxZoomLevel),
                    g = b.zoomLevel(),
                    t = b.middleXP,
                    m = b.middleYP,
                    n = b.realHeight / b.mapHeight,
                    s = b.realWidth / b.mapWidth,
                    t = (b.zoomX() - t * s) * (l / g) + t * s,
                    m = (b.zoomY() - m * n) * (l / g) + m * n;.1 < Math.abs(l - g) && (b.zoomTo(l, t, m, !0), b.mapWasPinched = !0, clearTimeout(b.pinchTO))
            }
            b.previousDistance = h
        }
        b.isDragging && (b.hideBalloon(), b.positionChanged(), k.translate(k.x + (e - c), k.y + (f - d), k.scale), b.updateSmallMap(), a && a.preventDefault && a.preventDefault());
        b.previuosMouseX = e;
        b.previuosMouseY = f
    },
    selectObject: function(a) {
        ...
        } else b.returnInitialColorReal(a); if (e = a.groupId)
            for (c = b.getGroupById(e), l = 0; l < c.length; l++)
                if (h = c[l], h.isOver = !1, e = h.displayObject)
                    if (k = h.selectedColorReal, void 0 !== f && e.setAttr("stroke", f), void 0 !== k ? e.setAttr("fill", k) : b.returnInitialColor(h), "MapLine" == h.objectType && ((k = h.lineSvg) && k.setAttr("stroke", g), h = h.arrowSvg)) h.setAttr("fill", g), h.setAttr("stroke", g);
    ...
    zoomToRectangle: function(a, b, c, d) {
        var e = this.realWidth,
            ...
            g = this.mapSet.scale,
            k = this.zoomControl,
            e = AmCharts.fitToBounds(c / e > d / f ? .8 * e / (c * g) : .8 * f / (d * g), k.minZoomLevel, k.maxZoomLevel);
    ...
    zoomToLatLongRectangle: function(a, b, c, d) {
        var e = this.dataProvider,
            ...
            g = Math.abs(c - a),
            k = Math.abs(b - d),
            h = Math.abs(e.rightLongitude - e.leftLongitude),
    ...
    rollOverMapObject: function(a, b, c) {
        if (this.chartCreated) {
            ...
            if (!this.preventHover) {
                ...
                else if ((this.showBalloonOnSelectedObject || !this.checkIfSelected(a)) && !1 !== b && (e = this.balloon, b = a.colorReal, d = "", void 0 !== b && this.useObjectColorForBalloon || (b = e.fillColor), (e = a.balloonTextReal) && (d = this.formatString(e, a)), this.balloonLabelFunction && (d = this.balloonLabelFunction(a, this)), d && "" !== d)) {
                    var f, g;
    ...
    parseXMLNode: function(a, b, c, d) {
        ...
        if (c) {
            ...
            for (e = 0; e < k; e++) {
                f = c.childNodes[e];
                var h = f.nodeName,
                    l = f.nodeValue ? this.trim(f.nodeValue) : "",
                    m = !1;
                f.attributes && 0 < f.attributes.length && (m = !0);
                if (0 !== f.childNodes.length || "" !== l || !1 !== m)
                    ...
                    var n;
                    void 0 !== a[b][h] ? void 0 === a[b][h].length ? (n = a[b][h], a[b][h] = [], a[b][h].push(n), a[b][h].push({}), n = a[b][h][1]) : "object" == typeof a[b][h] && (a[b][h].push({}), n = a[b][h][a[b][h].length - 1]) : (a[b][h] = {}, n = a[b][h]);
                    if (f.attributes && f.attributes.length)
                        for (l = 0; l < f.attributes.length; l++) n[f.attributes[l].name] = f.attributes[l].value;
...
AmCharts.ZoomControl = AmCharts.Class({
    ...
    init: function(a, b) {
        ...
        if (f) {
            ...
            q.push(r.set);
            var E = Math.log(D / u) / Math.log(x) + 1,
                f = B / E,
                p;
...
AmCharts.SmallMap = AmCharts.Class({
    ...
    init: function(a, b) {
        ...
        var f = c.borderThickness,
            g = c.borderColor,
            k = AmCharts.rect(b, c.width + f, c.height + f, c.backgroundColor, c.backgroundAlpha, f, g, c.borderAlpha);
        k.translate(-f / 2, -f / 2);
        e.push(k);
        k.toBack();
        var h, l, k = c.minimizeButtonWidth,
        ...
        n.setIcon(a.pathToImages + "arrowUp.gif", k);
...
AmCharts.AreasProcessor = AmCharts.Class({
    ...
    process: function(a) {
        ...
        for (e = 0; e < d; e++) {
            ...
            if (m && (c = m.area, p = m.title, f.enTitle = m.title, p && !f.title && (f.title = p), (m = b.language) ? (p = AmCharts.mapTranslations) && (m = p[m]) && m[f.enTitle] && (f.titleTr = m[f.enTitle]) : f.titleTr = void 0, c)) {
                f.displayObject = c;
    ...
    updateAllAreas: function() {
        ...
        for (m = 0; m < g.length; m++)
            if (h = g[m], void 0 != c && h.setAttr("fill", c), isNaN(d) || h.setAttr("fill-opacity", d), void 0 != e && h.setAttr("stroke", e), isNaN(f) || h.setAttr("stroke-opacity", f), h.setAttr("stroke-width", b.outlineThickness), k.getAreasFromMap && !l[h.id]) {
...
AmCharts.ImagesProcessor = AmCharts.Class({
    ...
    createImage: function(a, b) {
        ...
        var k = g.color,
            ...
            D = g.labelColor,
            y = g.labelFontSize,
            E = g.bringForwardOnHover,
            r = g.labelRollOverColor,
            q = g.selectedLabelColor;
        a.index = b;
        a.chart = c;
        ...
        if (q) {
            ...
            v.translate(x, l, g)
        } else r ? (isNaN(s) && (s = 10), isNaN(u) && (u = 10), v = d.image(r, 0, 0, s, u), v.node.setAttribute("preserveAspectRatio", "none"), v.setAttr("opacity", k), a.centeredReal && (x = isNaN(a.right) ? -s / 2 : s / 2, l = isNaN(a.bottom) ? -u / 2 : u / 2, v.translate(x, l))) : D && (v = d.path(D), n = v.getBBox(), a.centeredReal ? (x = -n.x * g - n.width * g / 2, isNaN(a.right) || (x = -x), l = -n.y * g - n.height * g / 2, isNaN(a.bottom) || (l = -l)) : x = l = 0, v.translate(x, l, g), v.x = x, v.y = l);
        ...
        z = null;
        void 0 !== a.label && (z = AmCharts.text(d, a.label, a.labelColorReal, c.fontFamily, a.labelFontSizeReal, a.labelAlign), v = a.labelBackgroundAlpha, (k = a.labelBackgroundColor) && 0 < v && (h = z.getBBox(), d = AmCharts.rect(d, h.width + 16, h.height + 10, k, v), p.push(d), a.labelBG = d), a.imageLabel = z, p.push(z));
    ...
    process: function(a) {
        ...
        for (q = 0; q < b.length; q++) {
            ...
            if (p.remainVisible || c.selectedObject == p.parentObject) {
                ...
                var A = this.processCoordinates(p.x, c.realWidth),
                    G = this.processCoordinates(p.y, c.realHeight),
                    N = p.longitudes,
                    Q = p.latitudes,
                    H = N.length,
                    O;
                if (0 < H)
                    for (A = [], O = 0; O < H; O++) A.push(c.longitudeToCoordinate(N[O]));
                H = Q.length;
                if (0 < H)
                    for (G = [], O = 0; O < H; O++) G.push(c.latitudeToCoordinate(Q[O]));
                if (0 < A.length) {
                    AmCharts.dx = 0;
                    AmCharts.dy = 0;
                    ...
                    if ("none" != J) {
                        ...
                        w = AmCharts.polygon(r, [-K / 2, 0, K / 2], [1.5 * K, 0, 1.5 * K], L, M, 1, L, M);
                        z.push(w);
                        w.translate(P, R);
                        w.rotate(C);
...
AmCharts.ValueLegend = AmCharts.Class({
    ...
    init: function(a, b) {
        ...
        if (this.showAsGradient) c = AmCharts.rect(b, this.width, this.height, [c, d], 1, this.borderThickness, this.borderColor, 1, 0, 0), c.translate(0, g), f.push(c);
            ...
            for (h = this.width / e, l = 0; l < e; l++) m = AmCharts.getColorFade(c, d, 1 * l / (e - 1)), m = AmCharts.rect(b, h, this.height, m, 1, this.borderThickness, this.borderColor, 1), m.translate(h * l, g), f.push(m);
        d = c = 0;
        e = f.getBBox();
