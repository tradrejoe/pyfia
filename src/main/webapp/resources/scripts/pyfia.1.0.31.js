...
pf.comp.panel.chart = function(cfg) {
	this.chart = function() {
		try {
			...
			$('#'+this.pchart.id).width(tmpwidth).height(tmpheight).css({padding: '0px'});
			var chart = AmCharts.makeChart( this.pchart.id, {
			  "type": "serial",
			  "theme": "light",
			  ...
			  "dataProvider": pyfia.get_amchart_candlestick_data(pyfia.data) 
			} );
			//performance issue
		    /*this.fusionChart = new FusionCharts({
				"type": "candlestick",
				"renderAt": this.pchart.id,
				"width": tmpwidth + "",
				"height": tmpheight + "",
				"dataFormat": "json",
				"dataSource": pyfia.get_fusioncharts_data(pyfia.data)
				});
			this.fusionChart.render();*/		
			//deprecated: highcharts
			/*$('#chartPanelContent').highcharts('StockChart', {
	            rangeSelector : {
	                selected : 1
	            },
	            title : {
		    ...
		    this.chart.pathToImages = "/resouces/scripts/amcharts/images/";
		    this.dataSet = new AmCharts.DataSet();
		    this.dataSet.color = "#0099CC";
		    this.dataSet.dataProvider = pyfia.get_chart_data();
		    this.dataSet.fieldMappings = [{fromField:"val", toField:"value"}];
		    this.dataSet.categoryField = "date";
		    ...
		    this.panelsSettings.startDuration = 1;
		    this.chart.panelsSettings = this.panelsSettings;
		    this.graph = new AmCharts.StockGraph();
		    this.graph.bullet = "round";
		    this.graph.valueField = "value";
		    this.graph.title = pyfia.data.clazz;
		    this.stockPanel.addStockGraph(this.graph);
		    this.chart.write("chartPanelContent");*/
			/*this.$super.createToolbar();
			$('#'+this.title.id).css({width:'99%',height: 20, position: 'absolute', top: 0, left: 0, clear: 'both', zIndex: 999, 'border-bottom':'1px inset #666666;'});
			this.title.style.top = '0px';
			this.title.style.left = '0px';
			this.title.style.width = '99%';*/
		} catch(err) {
