...
public class FincastProcessor implements IForecastProcessor {
	...
	public FincastResponse executeSingleThreaded(IForecastRequest request) {
		...
		CacheFinCls cacheFinCls = CacheFinCls.getInstance(clsDS, apd);
	...
	public FincastResponse executeSingleThreadedSvm(IForecastRequest request) {
		...
		try {
			...
			resp.setPredictionString(respr.getPredictionString().toLowerCase().indexOf("nan")>-1 ? "0":respr.getPredictionString());
	...
	public FincastResponse executeSingleThreadedRF(IForecastRequest request, Object... params) {
		...
		try {
			...
			String out = Evaluation.evaluateModel(new RandomForest(), new String[]{"-p", "0", "-z", ""}, freq.getTrainingSrc(), freq.getTestSrc());
			resp.setPredictionProb(0.5);
			resp.setPredictionString(out);
			//logger.debug("FincastResponse_R=" + resp);
			resp.setPredictionString(resp.getPredictionString().toLowerCase().indexOf("nan")>-1 ? "0":resp.getPredictionString());
		...
		//testCorrelations(String[] symbols, int[] forecast_ranges, int[] history, String[] indices)
		//correlation graph links
		if (params.length==0) return resp;
		try {
			List<CorrGraphLink> lnks = testCorrelations(new String[]{req.getClazz()}, 
					...
					freq.getAttributesList().toArray(new String[]{}),
					DateUtils.getDateNotime());
				...
				new Runnable() {
					public void run() {
						...
						try {
							...
							resp = null;
							resp.setStatus(FincastResponse.STATUS_DONE);
							//logger.debug("After eval model, requestId= " + freq.getRequestId() + "; resp=" + resp);
							if (resp!=null)
								cacheFincastResponse.put(requestId, resp);
						} catch(Exception e) {
							e.printStackTrace();
							logger.error("error", e);
							cacheFincastResponse.setStatus(requestId, "Error: " + e.getMessage() + ". Stack: " + ExceptionUtil.getStack(e));
						}
					}
				}
			);
	...
	public List<CorrGraphLink> testCorrelations(String[] symbols, int[] forecast_ranges, int[] history, String[] indices, Date dt) {
...
//					} catch(InterruptedException e)  {}
	...
	public static void main(String[] args) {
		try {
			DataSourceWrapper trainw = new DataSourceWrapper("AAPL", new String[]{"^IRX", "^NWX", "^SOX", "AAPL"}, new double[][]{
					{1.0,0.024,0.022,0.018,-0.014},
					{-1.0,0.034,0.042,0.041,-0.034},
					{0.0,0.018,0.027,0.02,-0.012},
					{0.0,0.004,0.016,0.031,0.038},
					{0.0,0.014,0.022,0.053,0.018},
					{0.0,-0.002,0.0,0.047,0.041},
					{-1.0,-0.014,0.013,0.047,0.02},
					{-1.0,-0.017,-0.005,0.038,0.031},
					{-0.5,0.001,-0.014,0.026,0.031},
					{-0.5,-0.004,-0.004,0.015,0.053},
			...
			DataSourceWrapper testw = new DataSourceWrapper("AAPL", new String[]{"^IRX", "^NWX", "^SOX", "AAPL"}, new double[][]{
					{-1.0,0.009,0.039,-0.014,0.01}
			});
			System.out.println(Evaluation.evaluateModel(new RandomForest(), new String[]{"-p", "0", "-z", "0"}, trainw.toDataSource(), testw.toDataSource()));
		} catch(Exception e) {
			e.printStackTrace();
		}
	}
