...
public class fcsty extends Applet {
	...
	void button_run_clicked(String args)
	{
		...
		else if(param.svm_type == fcsparm.EPSILON_SVR ||
			param.svm_type == fcsparm.NU_SVR)
		{
			if(param.gamma == 0) param.gamma = 1;
			prob.x = new fcsnd[prob.l][1];
			for(int i=0;i<prob.l;i++)
			{
				point p = point_list.elementAt(i);
				prob.x[i][0] = new fcsnd();
				prob.x[i][0].index = 1;
			...
			fcsmod model = fcs.svm_train(prob, param);
			fcsnd[] x = new fcsnd[1];
			x[0] = new fcsnd();
			x[0].index = 1;
			int[] j = new int[XLEN];
			...
			for (int i = 0; i < XLEN; i++)
			{
				...
				j[i] = (int)(YLEN*fcs.svm_predict(model, x));
			}
		...
		else
		{
			...
			for (int i = 0; i < XLEN; i++)
				for (int j = 0; j < YLEN ; j++) {
					...
					double d = fcs.svm_predict(model, x);
					if (param.svm_type == fcsparm.ONE_CLASS && d<0) d=2;
	...
	void button_save_clicked(String args)
	{
		...
		try {
			...
			int n = point_list.size();
			if(svm_type == fcsparm.EPSILON_SVR || svm_type == fcsparm.NU_SVR)
			{
				for(int i=0;i<n;i++)
				{
					point p = point_list.elementAt(i);
	...
	{
		new AppletFrame("svm_toy",new fcsty(),500,500+50);
	}
}
