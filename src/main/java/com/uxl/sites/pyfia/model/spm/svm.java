...
abstract class Kernel extends QMatrix {
	...
	void swap_index(int i, int j)
	{
		do {svm_node[] ii=x[i]; x[i]=x[j]; x[j]=ii;} while(false);
...
public class svm {
	...
	public static void svm_cross_validation(svm_problem prob, svm_parameter param, int nr_fold, double[] target)
	{
		...
		for(i=0;i<nr_fold;i++)
		{
			    ...
			    param.svm_type == svm_parameter.NU_SVC))
			{
				...
				for(j=begin;j<end;j++)
					target[perm[j]] = svm_predict_probability(submodel,prob.x[perm[j]],prob_estimates, new FincastResponse());
			}
			else
				for(j=begin;j<end;j++)
	...
	//TODO: get prob?
	public static double svm_predict_probability(svm_model model, svm_node[] x, double[] prob_estimates, FincastResponse resp)
	{
		    ...
		    model.probA!=null && model.probB!=null)
		{
			...
			int prob_max_idx = 0;
			resp.setPredictionLabel(model.label[prob_max_idx]);
			resp.setPredictionProb(prob_estimates[prob_max_idx]==0.0 ? 0.5 : prob_estimates[prob_max_idx]);			
			for(i=1;i<nr_class;i++)
				if(prob_estimates[i] > prob_estimates[prob_max_idx]) {
					prob_max_idx = i;
					resp.setPredictionLabel(model.label[i]);
					resp.setPredictionProb(prob_estimates[i]==0.0 ? 0.5 : prob_estimates[i]);
				}
			resp.setModelstring(model+"");
	...
	{
		if (model_file_name==null) return;
	...
	public static FincastResponse exec(FincastRequest req, Object... args) throws IOException {
		FincastResponse resp = req.prepareResponse();
		svm_train t = new svm_train();
		t.exec(req, args!=null && args.length>0 && args[0] instanceof Boolean && ((Boolean)args[0]).booleanValue());
		ByteArrayOutputStream out = svm_predict.run(t.getModel(), req.getTestSet(), resp);
		resp.setPredictionString(out.toString());
		return resp;
