...
public class CacheFinCls extends TreeMap<String, List<FinCaseList>> {
	...
	public static final long DEFAULT_PDN_TIMEOUT = 5;
	public static final TimeUnit DEFAULT_PDN_TIMEOUT_UNIT = TimeUnit.MINUTES;

	Logger logger = Logger.getLogger(CacheFinCls.class);
	static CacheFinCls instance = new CacheFinCls();
	...
	public static CacheFinCls getInstance(AbstractPriceDownloader pd) {
		return instance;
	}
	...
	public List<FinCaseList> fetch(String cls, Object... flags) {
		...
		try {
//			boolean locked = lock.tryLock(DEFAULT_PDN_TIMEOUT, DEFAULT_PDN_TIMEOUT_UNIT);
//			if (locked) {
				list = instance.get(cls);
				// logger.debug("CacheFinCls::get('"+cls+"'), after mem qry, list=" +
				...
				// logger.debug("CacheFinCls::get('"+cls+"'), after db query and pd, list=" +
				// JsonUtil.serialize(list));
				...
				for (FinCaseList l : list) {
					...
					if (ldt.compareTo(dt) == -1) {
						...
						if (tmplist == null)
							tmplist = pd.getFinCaseList(cls, lcal, GregorianCalendar.getInstance(), flags);
						if (tmplist != null && tmplist.size() > 0 && tmplist.get(0).size() > 0) {
							...
							for (FinCaseList tl : tmplist) {
								logger.debug("Saving latest data in db for " + cls); // + ", tmp list tl=" +
																						// JsonUtil.serialize(tl));
...
//			}
			...
			logger.error("CacheFinCls::fetch(), cannot get data for clz " + cls);
			ex.printStackTrace();
			return list;
		}
		return list;
	...
	public List<FinCaseList> getPassReturns(String cls, int lag, Date start, int history, Boolean... flags)
			throws Exception {
		if (lag < 0)
		...
		if (list != null && list.size() > 0 && list.get(0).size() > 0) {
			...
			for (FinCaseList lst : list) {
				if (!sd && FinCaseSDList.class.isInstance(lst))
					continue;
	...
	public FinCaseList getFutureReturns(String cls, int lag, Date start, int history, Boolean... flags)
			throws Exception {
		if (lag < 0)
			throw new NegativeLagException();
		...
		if (list != null && list.size() > 0 && list.get(0).size() > 0) {
			...
			for (FinCaseList lst : list) {
				if (FinCaseSDList.class.isInstance(lst))
					continue;
				Set<Date> keys = lst.keySet();
				Date[] dates = (Date[]) keys.toArray(new Date[0]);
				for (int i = dates.length - 1; i >= 0; i--) {
	...
	public FincastRequest prepareFincastRequest(ForecastRequest in) throws Exception {
		...
		for (int a = 0; a < attra.length; a++) {
			String tmpa = attra[a];
			if (tmpa == null || tmpa.trim().equals(""))
				continue;
			status = "Getting historical data for " + tmpa + ".";
			cacheFincastResponse.setStatus(requestId, status);
			logger.debug(status);
			List<FinCaseList> tmpfcl = getPassReturns(tmpa, out.getForecastRange(), in.getCase(), in.getHistory(),
					in.isSd());
		...
		for (FinCaseList cval : cvallist) {
			if (FinCaseSDList.class.isInstance(cval))
				continue;
...
//		logger.debug(bufTest);
	...
	public FincastRequest prepareFincastRequestSvm(ForecastRequest in, Object... args) throws Exception {
		...
		boolean regression = (args != null && args.length > 0 && args[0] instanceof Boolean
		...
		for (startRowInFinCls = datesCls.length - 1; startRowInFinCls >= 0; startRowInFinCls--) {
			...
			if (hasWholeTestRow) {
				...
				bufTest += (regression ? "0.01 " : "+1 ") + bufTestRow + "\r\n";// + new
																				// Character((char)StreamTokenizer.TT_EOF);
				break;
			}
		}
	...
	public FincastRequest prepareFincastRequestRF(ForecastRequest in, Object... params) throws Exception {
		...
		CacheFincastResponse cacheFincastResponse = CacheFincastResponse.getInstance();
		String requestId = in.getRequestId();
		logger.debug(String.format("In CacheFinCls::prepareFincastRequestRF(), test date=%1$s",
				DateUtils.sdf.format(in.getCase())));
		FincastRequest out = new FincastRequest(in);
		out.setRequestId(in.getRequestId());
		...
		for (FinCaseList cval : cvallist) {
			...
			tmpCaseValues.addAll(
					CollectionUtil.subset(tmpColValues, tmpColValues.size() - in.getHistory(), tmpColValues.size()));
			if (FinCaseOList.class.isInstance(cval))
				out.setCaseValuesO(tmpCaseValues);
			else if (FinCaseHList.class.isInstance(cval))
				out.setCaseValuesH(tmpCaseValues);
			else if (FinCaseLList.class.isInstance(cval))
				out.setCaseValuesL(tmpCaseValues);
			else if (FinCaseSDList.class.isInstance(cval))
				out.setCaseValuesSD(tmpCaseValues);
			else
				out.setCaseValues(tmpCaseValues);
		}
