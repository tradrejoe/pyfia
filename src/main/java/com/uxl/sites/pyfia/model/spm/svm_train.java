...
class svm_train {
	...
	public static void setSvm_print_null(svm_print_interface svm_print_null) {
		svm_train.svm_print_null = svm_print_null;
	}
	 ...
	 * @throws IOException
	 */
	private void run(String argv[], InputStream... in) throws IOException
	{
	...
	public void exec(FincastRequest freq, boolean reg) throws IOException {
		//polynomial kernal
		run(reg ? new String[]{"-s", "3", "-t", "1"} : new String[]{"-s", "1", "-t", "1"}, freq.getTrainingSet());
	}
	...
	private svm_parameter getDefaultParam() {
	...
	private void read_problem(InputStream... in) throws IOException
	{
		BufferedReader fp = new BufferedReader((in!=null && in.length>0) ? new InputStreamReader(in[0]) : 
			new FileReader(input_file_name));
		Vector<Double> vy = new Vector<Double>();
		Vector<svm_node[]> vx = new Vector<svm_node[]>();
		int max_index = 0;
		int MAX_EMPTY_LINES = 10;
		int empty_lines = 0;
		while(true)
		{
